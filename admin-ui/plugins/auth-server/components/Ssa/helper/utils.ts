...
export const logSsaCreation = async (payload: SsaFormValues, message?: string): Promise<void> => {
  try {
    ...
    logger.error('dev', 'Failed to log SSA creation:', error instanceof Error ? error : String(error))
  }
}
...
export const logSsaDeletion = async (
  ...
  try {
    ...
    logger.error('dev', 'Failed to log SSA deletion:', error instanceof Error ? error : String(error))
  }
}
