...
export function* getConfigMessage() {
  try {
    ...
    addAdditionalData(audit, FETCH, MESSAGE, {})
...
export function* editMessageConfig({ payload }) {
  try {
    const audit = yield* initAudit()
    addAdditionalData(audit, UPDATE, MESSAGE, payload)
    yield put(toggleMessageConfigLoader(true))
    const api = yield* newFunction()
    const token = yield select((state) => state.authReducer.token.access_token)
    const data = yield call(api.patchConfigMessage, {
      data: JSON.stringify(payload.action_data),
      token,
    })
...
export function* putConfigMessagePostgres({ payload }) {
  try {
    const audit = yield* initAudit()
    addAdditionalData(audit, UPDATE, `${MESSAGE}-${POSTGRES}`, payload)
...
export function* putConfigMessageRedis({ payload }) {
  try {
    const audit = yield* initAudit()
    addAdditionalData(audit, UPDATE, `${MESSAGE}-${REDIS}`, payload)
    yield toggleSaveConfigLoader(true)
