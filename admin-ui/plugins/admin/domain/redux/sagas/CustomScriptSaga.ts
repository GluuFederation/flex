...
import { SCRIPT } from '../../../utils/Resources'
import {
  CREATE,
  UPDATE,
  DELETION,
...
import ScriptApi from '../../../infrastructure/api/ScriptApi'
...
export function* getCustomScripts({ payload } : PayloadAction<any>) {
...
export function* getScriptsByType({ payload } : PayloadAction<any>) {
  const audit = yield* initAudit()
  try {
    addAdditionalData(audit, FETCH, SCRIPT, payload)
    const scriptApi = yield* newFunction()
...
export function* addScript({ payload } : PayloadAction<any>) {
  const audit = yield* initAudit()
  try {
...
export function* editScript({ payload } : PayloadAction<any>) {
  const audit = yield* initAudit()
  try {
...
export function* deleteScript({ payload } : PayloadAction<any>) {
  const audit = yield* initAudit()
  try {
