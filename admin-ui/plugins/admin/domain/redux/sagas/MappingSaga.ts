...
import {
  ...
  getMapping,
} from 'Plugins/admin/domain/redux/features/MappingSlice'
import { API_MAPPING } from '../../../utils/Resources'
...
import MappingApi from '../../../infrastructure/api/MappingApi'
...
export function* fetchMapping({ payload } : PayloadAction<any>) {
...
export function* updateMapping({ payload } : PayloadAction<any>) {
...
export function* addMapping({ payload } : PayloadAction<any>) {
  yield put(updatePermissionsLoading({ data: true }))
  try {
    const mappingApi = yield* newFunction()
    yield call(mappingApi.addMapping, payload.data)
    yield put(updateToast(true, 'success'))
...
export function* deleteMapping({ payload } : PayloadAction<any>) {
  yield put(updatePermissionsLoading({ data: true }))
  try {
    const mappingApi = yield* newFunction()
    yield call(mappingApi.deleteMapping, payload.data)
    yield put(updateToast(true, 'success'))
    yield put(getMapping({}))
  } catch (e) {
    yield put(updateToast(true, 'error'))
