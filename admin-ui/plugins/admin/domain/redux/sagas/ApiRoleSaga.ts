...
import {
  ...
  editRoleResponse,
  deleteRoleResponse
} from 'Plugins/admin/domain/redux/features/ApiRoleSlice'
import { API_ROLE } from '../../../utils/Resources'
import {
  CREATE,
  UPDATE,
  DELETION,
...
export function* getRoles({ payload } : PayloadAction<any>) {
...
export function* addRole({ payload } : PayloadAction<any>) {
  const audit = yield* initAudit()
  try {
    addAdditionalData(audit, CREATE, API_ROLE, payload)
    const roleApi = yield* newFunction()
    yield call(roleApi.addRole, payload.action.action_data)
...
export function* editRole({ payload } : PayloadAction<any>) {
  const audit = yield* initAudit()
  try {
    addAdditionalData(audit, UPDATE, API_ROLE, payload)
    const roleApi = yield* newFunction()
    yield call(roleApi.editRole, payload.action.action_data)
    yield put(updateToast(true, 'success'))
    yield put(getRolesAction({}))
    yield call(postUserAction, audit)
  } catch (e) {
...
export function* deleteRole({ payload } : PayloadAction<any>) {
  const audit = yield* initAudit()
  try {
