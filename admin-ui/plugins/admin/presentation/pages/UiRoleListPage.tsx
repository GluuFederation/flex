...
import RoleAddDialogForm from '../components/Roles/RoleAddDialogForm'
...
function UiRoleListPage({ apiRoles, permissions, loading, dispatch }) {
  ...
  const options = []
  const userAction = {}
  const pageSize: number = localStorage.getItem('paggingSize')
  ...
  const DetailsPanel = useCallback(
    (rowData) => <UiRoleDetailPage row={rowData} />,
    []
  )
  ...
  const EditComponent = useCallback(
    (rowData) => (
      ...
      >
        <option
          selected={String(rowData.rowData.deletable) == 'true' ? true : false}
          value={'true'}
        >
          true
        </option>
