...
const RoleAddDialogForm = ({ handler, modal, onAccept }) => {
  ...
  function handleStatus() {
    const value = (document.getElementById('api_role') as HTMLInputElement).value
    if (value.length >= 5) {
      setActive(true)
    } else {
      setActive(false)
  ...
  function handleAccept() {
    const roleData = {}
    roleData['role'] = (document.getElementById('api_role') as HTMLInputElement).value
    roleData['description'] = (document.getElementById('api_description') as HTMLInputElement).value
