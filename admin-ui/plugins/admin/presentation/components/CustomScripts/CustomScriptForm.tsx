...
import items from '../../../utils/ScriptTypes'
import GluuScriptErrorModal from "Routes/Apps/Gluu/GluuScriptErrorModal";
import { Alert, Button } from "reactstrap";
import ErrorIcon from '@mui/icons-material/Error';
import { getPropertiesConfig, scriptInitValues, scriptValidationSchema, submitScriptForm } from 'Plugins/admin/domain/useCases/CustomScriptUseCases'

interface TProps { item: any, scripts?: any, handleSubmit: any, viewOnly?: boolean }

function CustomScriptForm(props: TProps) {
  const { item, handleSubmit, viewOnly } = props
  ...
  function submitForm() {
    toggle();
    (document.getElementsByClassName('UserActionSubmitButton')[0] as HTMLElement).click()
  }

  const formik: any = useFormik({
    initialValues: scriptInitValues(item),
    validationSchema: scriptValidationSchema,

    onSubmit: (values: any) => {
      const reqBody = submitScriptForm(values, item)
