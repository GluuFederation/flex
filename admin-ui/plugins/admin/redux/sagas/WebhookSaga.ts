...
export function* getWebhooks({ payload }: { payload: WebhookActionPayload }): Generator<CallEffect | PutEffect | SelectEffect, any, any> {
...
export function* createWebhook({ payload }: { payload: WebhookActionPayload }): Generator<CallEffect | PutEffect | SelectEffect, any, any> {
  const audit: AuditLog = yield call(initAudit)
  try {
...
export function* deleteWebhook({ payload }: { payload: WebhookActionPayload }): Generator<CallEffect | PutEffect | SelectEffect, any, any> {
  const audit: AuditLog = yield call(initAudit)
  try {
...
export function* updateWebhook({ payload }: { payload: WebhookActionPayload }): Generator<CallEffect | PutEffect | SelectEffect, any, any> {
  const audit: AuditLog = yield call(initAudit)
  try {
...
export function* getFeaturesByWebhookId({ payload }: { payload: FeatureActionPayload }): Generator<CallEffect | PutEffect | SelectEffect, any, any> {
  const audit: AuditLog = yield call(initAudit)
  try {
    addAdditionalData(audit, FETCH, 'webhook', payload)
    const webhookApi: WebhookApi = yield call(newFunction)
...
export function* getWebhooksByFeatureId({ payload }: { payload: string }): Generator<CallEffect | PutEffect | SelectEffect, any, any> {
  const audit: AuditLog = yield call(initAudit)
  try {
...
export function* triggerWebhook({ payload }: { payload: TriggerWebhookPayload }): Generator<CallEffect | PutEffect | SelectEffect, any, any> {
  const audit: AuditLog = yield call(initAudit)
  try {
    const webhookApi: WebhookApi = yield call(newFunction)
    const featureToTrigger: string = yield select((state: RootState) => state.webhookReducer.featureToTrigger)
    const featureWebhooks: Webhook[] = yield select((state: RootState) => state.webhookReducer.featureWebhooks)
    const enabledFeatureWebhooks: Webhook[] = featureWebhooks?.filter((item: Webhook) => item.jansEnabled)
    ...
    if (all_succeded) {
      ...
      const errors = data?.body?.map((item: WebhookResponse) => !item.success && item)?.filter((err: any) => err)
    ...
    yield put(updateToast(true, 'error', error?.response?.body?.responseMessage || error.message || 'Unknown error'))
    yield put(setWebhookModal(true))
    yield put(setTriggerWebhookResponse(error?.response?.body?.responseMessage || error.message || 'Unknown error'))
    if (isFourZeroOneError(error)) {
      const jwt: string = yield select((state: RootState) => state.authReducer.userinfo_jwt)
      yield put(getAPIAccessToken(jwt))
    }
