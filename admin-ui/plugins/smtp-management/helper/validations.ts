...
export const getSmtpValidationSchema = (t: TFunction) =>
  Yup.object({
    host: Yup.string().trim().required(t('messages.smtp_host_required')),
    port: Yup.number()
    ...
    from_name: Yup.string().trim().required(t('messages.smtp_from_name_required')),
    ...
    smtp_authentication_account_username: Yup.string().when(
      ...
      (values, schema) =>
        values[0]
          ? schema.trim().required(t('messages.smtp_username_required'))
          : schema.nullable(),
    ),
