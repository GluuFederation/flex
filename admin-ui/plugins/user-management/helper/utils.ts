...
export const logUserCreation = async (_data: CustomUser, payload: CustomUser): Promise<void> => {
  try {
    ...
    logger.error('dev', 'Failed to log user creation:', error instanceof Error ? error : String(error))
  }
}
...
export const logUserUpdate = async (_data: CustomUser, payload: CustomUser): Promise<void> => {
  try {
    ...
    logger.error('dev', 'Failed to log user update:', error instanceof Error ? error : String(error))
  }
}
...
export const logUserDeletion = async (inum: string, userData?: CustomUser): Promise<void> => {
  try {
    ...
    logger.error('dev', 'Failed to log user deletion:', error instanceof Error ? error : String(error))
  }
}
