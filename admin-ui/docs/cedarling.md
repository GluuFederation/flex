...
## Policy-store management (in development)

> **⚠️ This section documents a temporary development shim that MUST be reverted before the
> feature is merged to `main`.** See the checklist at the end.

[GluuFederation/flex#2985](https://github.com/GluuFederation/flex/issues/2985) adds screens for
managing policy stores — upload with comments, a Policy Store History table (activate / download /
delete), and an in-browser `.cjar` Archive Explorer. Multiple policy stores are persisted in the
`adminUIPolicyStore` table and exactly one row carries `jansStatus = 'active'` at a time. That
active store is the one Phase 1 above loads into Cedarling at sign-in.

### API surface

The endpoints are generated by orval from the [`jans-config-api-14533`](https://github.com/JanssenProject/jans/blob/jans-config-api-14533/jans-config-api/plugins/docs/jans-admin-ui-plugin-swagger.yaml)
feature-branch swagger. They replace the previous single-store `GET`/`PUT` pair:

| Generated hook                | Method / path                                                                                                            |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `useGetAdminuiPolicyStore`    | `GET /admin-ui/security1/policyStore` (paged: `limit`, `startIndex`, `pattern`, `sortBy`, `sortOrder`, `fieldValuePair`) |
| `useCreateAdminuiPolicyStore` | `POST /admin-ui/security1/policyStore` (JSON, base64 `policyStore`)                                                      |
| `useEditAdminuiPolicyStore`   | `PUT /admin-ui/security1/policyStore/{inum}`                                                                             |
| `useDeleteAdminuiPolicyStore` | `DELETE /admin-ui/security1/policyStore/{inum}`                                                                          |
| `useSyncRoleToScopesMappings` | `POST /admin-ui/security/syncRoleScopesMapping` — **unchanged**, still on the legacy path                                |

Two upstream quirks to be aware of:

- **The `GET` response shape is ambiguous in the spec.** It declares `type: array` (so orval types
  it `AdminUIPolicyStore[]`) but its own example returns a paged envelope
  `{ start, totalEntriesCount, entriesCount, entries }`. Our data layer normalizes both shapes;
  the spec should be fixed upstream.
- **`Security` has no `delete` action.** [`resourceCatalog.ts`](../app/cedarling/constants/resourceCatalog.ts)
  declares `Security: ['read', 'write']`. The history page's Delete button needs `canDelete`, which
  needs a matching Cedar policy in the shipped policy store — a change on the Janssen side, not
  something the UI can grant itself.
...
Archives are read and written by [`app/utils/cjarArchive.ts`](../app/utils/cjarArchive.ts), which
parses the zip container directly and delegates DEFLATE to the platform's `DecompressionStream` /
`CompressionStream`. That avoids adding a zip dependency; the cost is that `readArchive` and
`writeArchive` are async, and a browser without those constructors can read stored entries but not
compressed ones.

### Why `security1`

The feature-branch backend registers the new policy-store resource under `/admin-ui/security1` so
it can run alongside the legacy `/admin-ui/security` resource while both are supported. Upstream
will drop the `1` once development completes. The swagger file itself says `security` (no `1`), so
[`script/patch-adminui-policystore-path.ts`](../script/patch-adminui-policystore-path.ts) rewrites
just the two `policyStore` paths in the merged spec during `npm run api:orval`, driven by
`POLICY_STORE_PATH_SUFFIX` in [`app/constants/policyStore.ts`](../app/constants/policyStore.ts).

### Revert checklist — before raising the PR

Shipping the shim breaks `npm run api:orval` once the feature branch is deleted, and points the
Phase 1 bootstrap at a path released backends do not serve — which denies every permission and
blanks the entire admin UI, not just this feature. Do this as the last commit before the PR:

1. Confirm with Janssen that the final path is plain `/admin-ui/security/policyStore`.
2. `app/constants/policyStore.ts` — set `POLICY_STORE_PATH_SUFFIX` from `'1'` to `''`.
3. `openapi-merge.json` — point the admin-ui plugin swagger URL back at `main`.
4. Delete `script/patch-adminui-policystore-path.ts` and its step in the `api:orval` chain in `package.json`.
5. Re-run `npm run api:orval`, then retest sign-in end to end.
