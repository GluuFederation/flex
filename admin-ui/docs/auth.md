...
**Error / network handling.** Every catch block in [`licenseListener.ts`](../app/redux/listeners/licenseListener.ts) turns the failure into a message with `getLicenseErrorMessage` and dispatches `setLicenseError`, so the UI can show what went wrong. The license-config check goes further: `isAuthFailure` treats a 401 or 403 (`AUTH_FAILURE_STATUSES`) as a hard sign-out and dispatches `handleSessionExpired`, because that means the OIDC token is valid but the user lacks the role to call the license endpoints. Backend reachability is tracked separately: when the API-protection-token call fails, [`authListener.ts`](../app/redux/listeners/authListener.ts) dispatches `setBackendStatus` with the status code and message, and `ApiKeyRedirect` renders the global `GluuServiceDownModal`.
