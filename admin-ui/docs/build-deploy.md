...
The values from these files become available in the build as `process.env.<NAME>` (Vite rewrites these statically at build time) and are also read by the dev server's `admin-ui:env-config` plugin to populate `env-config.js` at runtime — see [Runtime env injection](#runtime-env-injection) below. Full variable list in [onboarding.md](./onboarding.md#variables).
...
The production bundle is split into chunks so the browser can cache vendor code separately from app code — changing one component file shouldn't invalidate the entire React download. Chunking is configured declaratively in [`vite.config.ts`](../vite.config.ts) through `output.codeSplitting.groups`, with the `FEATURE_GROUPS` constant listing the package buckets.
...
> [!WARNING]
> **Don't use the `manualChunks` callback form** on Vite 8 / Rolldown. It duplicates React across chunks and produces React error #130 at runtime ("Element type is invalid"). Always use the declarative `output.codeSplitting.groups` form. `vendor-react` must come first in the groups list and stay runtime-only. We have hit this bug before — if a future PR proposes switching to a `manualChunks` callback for any reason, the answer is no.
...
| Dev (Vite)            | The `admin-ui:env-config` plugin serves it via `configureServer` from `.env.development`, or an empty no-op |
