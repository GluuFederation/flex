      ...
      if (response) {
        ...
        AuthorizationServiceConfiguration.fetchFromIssuer(
          ...
          .then((ujwt) => {
            if (!userinfo) {
              dispatch(
                getUserInfoResponse({ userinfo: jwt_decode(ujwt), ujwt: ujwt }),
              )
              ...
              if (
                ...
                userinfo.jansAdminUIRole.length == 0
              ) {
                ...
                const sessionEndpoint = `${
                  authConfigs.endSessionEndpoint
                }?state=${state}&post_logout_redirect_uri=${localStorage.getItem(
