...
| global.cnDocumentStoreType | string | `"JCA"` | Document store type to use for shibboleth files JCA or LOCAL. Note that if JCA is selected Apache Jackrabbit will be used. Jackrabbit also enables loading custom files across all services easily. |
...
| global.jackrabbit.cnJackrabbitAdminId | string | `"admin"` | Jackrabbit admin uid. |
| global.jackrabbit.cnJackrabbitCluster | bool | `false` | Boolean flag if enabled will enable jackrabbit in cluster mode with Postgres. |
| global.jackrabbit.cnJackrabbitPostgresDatabaseName | string | `"jackrabbit"` | Jackrabbit postgres database name. |
| global.jackrabbit.cnJackrabbitPostgresHost | string | `"postgresql.postgres.svc.cluster.local"` | Postgres url |
| global.jackrabbit.cnJackrabbitPostgresPort | int | `5432` | Jackrabbit Postgres port |
| global.jackrabbit.cnJackrabbitPostgresUser | string | `"jackrabbit"` | Jackrabbit Postgres uid |
| global.jackrabbit.cnJackrabbitSyncInterval | int | `300` | Interval between files sync (default to 300 seconds). |
| global.jackrabbit.cnJackrabbitUrl | string | `"http://jackrabbit:8080"` | Jackrabbit internal url. Normally left as default. |
