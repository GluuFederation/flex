...
/**
 * An app. scoped bean that encapsulates interactions with an oxd-server.
 * Contains methods depicting the steps of the authorization code flow of OpenId
 * Connect spec
 * 
 * @author jgomer
 */
...
public class OxdService {
	...
	/*
	 * The list of scopes required to be able to inspect the claims needed. See
	...
	public boolean initialize(OxdSettings oxdConfig) {
		...
		if (oxdConfig == null) {
			...
			if (oxdConfig.getPort() <= 0 || missing) {
				logger.error(
						"The following must be present in configuration file: host, port, redirect URI, post logout URI, and front channel logout URI");
			} else {
				...
				try {
					if (persistenceService.getDynamicClientExpirationTime() != 0) {
						Optional<String> oxdIdOpt = Optional.ofNullable(oxdConfig.getClient())
								.map(OxdClientSettings::getOxdId);
					...
					logger.warn(
							"Users will not be able to login until a new sucessful attempt to refresh oxd-associated "
	...
	public OxdClientSettings doRegister() throws Exception {
		...
		logger.info("Setting oxd configs (host: {}, port: {},  post logout: {})",
				new String[] { config.getHost(), String.valueOf(config.getPort()), config.getPostLogoutUri() });
		...
		try {
			...
			computedSettings = new OxdClientSettings(clientName, setup.getOxdId(), setup.getClientId(),
	...
	/**
	 * Returns a string with an autorization URL to redirect an application (see
	 * OpenId connect "code" flow)
	 * 
	 * @param acrValues List of acr_values. See OpenId Connect core 1.0 (section
	 *                  3.1.2.1)
	 * @param prompt    See OpenId Connect core 1.0 (section 3.1.2.1)
	 * @return String consisting of an authentication request with desired
	 *         parameters
	...
	public String getAuthzUrl(List<String> acrValues, String prompt) throws Exception {
		...
		GetAuthorizationUrlResponse resp = restResponse(cmdParams, "get-authorization-url", getPAT(),
				GetAuthorizationUrlResponse.class);
	...
	public Pair<String, String> getTokens(String code, String state) throws Exception {
		...
		GetTokensByCodeResponse resp = restResponse(cmdParams, "get-tokens-by-code", getPAT(),
				GetTokensByCodeResponse.class);
	...
	private <T> T restResponse(IParams params, String path, String token, Class<T> responseClass) throws Exception {
		...
		ResteasyWebTarget target = client
				.target(String.format("https://%s:%s/%s", config.getHost(), config.getPort(), path));
